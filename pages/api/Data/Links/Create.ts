...
// Extend payload interface to include Email and PhotoUrl
...
export default async function addLink(
  ...
  try {
    const {
      ...
      Description,
      PhotoUrl,
    } = req.body as Partial<LinkPayload>;

    // Validate required fields
    if (!Email || !Url || !LinkName || !Description) {
      return res
        .status(400)
        .json({ message: 'Email, Url, LinkName, and Description are required.' });
    }
    ...
    return res
      .status(201)
      .json({ ...newDoc, _id: result.insertedId });
  } catch (err) {
