...
const UsersTable: React.FC<UsersCardProps> = ({ posts }) => {
  ...
  const groupedData: Record<string, Post[]> = posts.reduce((acc: Record<string, Post[]>, post: Post) => {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    const filteredGroups = Object.entries(groupedData).filter(([_, entries]) => {
      ...
      const filteredEntries = entries.filter((post) => {
        ...
        const matchesDateRange =
          (!start || !postDate || postDate >= start) && (!end || !postDate || postDate <= end);
    ...
    const mapped = filteredGroups.map(([companyName, entries]) => {
      ...
      const totalSales = filteredEntries.reduce((sum, post) => {
        const value = typeof post.actualsales === "string" ? parseFloat(post.actualsales) : post.actualsales;
  ...
  const grandTotal = filteredAndSortedData.reduce((sum, item) => sum + item.totalSales, 0);

  const averageAchievement = filteredAndSortedData.length > 0
  ...
  const handleExportToExcel = async () => {
    ...
    filteredAndSortedData.forEach(({ companyName, typeClients, totalSales, averageSales }) => {
      worksheet.addRow({
        companyName,
        typeClients,
        totalSales,
      ...
      <div className="mb-4 flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <label htmlFor="typeClientFilter" className="font-semibold text-xs whitespace-nowrap">
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map(({ companyName, totalSales, typeClients, averageSales, targetQuota }) => (
                <tr key={companyName} className="bg-white hover:bg-gray-50">
                  ...
                  <td className={`px-6 py-4 text-xs font-semibold ${totalSales !== 0 &&
                    !isNaN(Number(String(targetQuota).replace(/,/g, ""))) &&
                    (totalSales / (Number(String(targetQuota).replace(/,/g, "")) / 100)) >= 100
                    ? "text-green-600"
                    : "text-red-600"
                    }`}>
                    {totalSales !== 0 && !isNaN(Number(String(targetQuota).replace(/,/g, "")))
                      ? `${(totalSales / (Number(String(targetQuota).replace(/,/g, "")) / 100)).toFixed(2)}%`
                      : "N/A"}
                  </td>
                </tr>
              ))
