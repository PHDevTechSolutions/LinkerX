...
const UsersCard: React.FC<UsersCardProps> = ({ posts, handleEdit, handleDelete, Role, Department }) => {
  ...
  const filteredUsers = useMemo(() => {
    return updatedUser.filter((post) => {
      ...
      if (Role === "Admin" && post.Role !== "Manager") return false;
      if (Role === "Super Admin" && !["Manager", "Admin"].includes(post.Role)) return false;
      return true;
    });
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ModuleSales/UserManagement/ManagerDirector/Bulk-Delete`, {
        method: "DELETE",
