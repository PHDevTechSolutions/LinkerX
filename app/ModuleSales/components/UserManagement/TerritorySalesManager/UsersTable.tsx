...
const UsersCard: React.FC<UsersCardProps> = ({ posts, handleEdit, handleDelete, Role, Department }) => {
  ...
  const filteredUsers = useMemo(() => {
    return updatedUser.filter((post) => {
      if (Department && post.Department !== Department) return false;
      if (Role === "Admin" && post.Role !== "Territory Sales Manager") return false;
      if (Role === "Super Admin" && !["Territory Sales Manager"].includes(post.Role)) return false;
      return true;
    });
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ModuleSales/UserManagement/TerritorySalesManager/Bulk-Delete`, {
        method: "DELETE",
        ...
        <div className="mb-4 p-2 bg-gray-100 rounded-md text-xs flex justify-between items-center">
          <div className="flex items-center">
            <input
              ...
              onChange={() => {
                if (selectedUsers.size === filteredUsers.length) {
                  setSelectedUsers(new Set()); // Unselect all
                } else {
                  setSelectedUsers(new Set(filteredUsers.map(user => user._id))); // Select all
                }
              }}
              className="w-4 h-4"
            />
            ...
            <span className="ml-4 font-semibold text-gray-700">
              Selected: {selectedUsers.size} / {filteredUsers.length}
            </span>
          </div>
          <button
            onClick={handleBulkDelete}
            className="px-3 py-1 bg-red-600 text-white rounded-md hover:bg-red-700 text-xs"
            disabled={selectedUsers.size === 0} // Disable button if no users are selected
          >
            Bulk Delete
          </button>
        </div>
