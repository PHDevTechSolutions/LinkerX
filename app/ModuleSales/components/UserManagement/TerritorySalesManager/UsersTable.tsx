...
const UsersCard: React.FC<UsersCardProps> = ({ posts, handleEdit, handleDelete, fetchUsers }) => {
  ...
  useEffect(() => {
    if (bulkTransferMode) {
      fetch("/api/manager?Role=Manager")
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleBulkTransfer = useCallback(async () => {
    if (selectedUsers.size === 0 || !selectedManager) return;
    try {
      const response = await fetch(`/api/ModuleSales/UserManagement/TerritorySalesManager/Bulk-Transfer`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userIds: Array.from(selectedUsers),
          managerReferenceID: selectedManager
        }),
      });
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {managerList.map((manager) => (
                  <option key={manager._id || manager.ReferenceID} value={manager.ReferenceID}>
