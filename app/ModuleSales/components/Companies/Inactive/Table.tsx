...
const UsersTable: React.FC<UsersCardProps> = ({ posts, handleEdit }) => {
  ...
  // ✅ Group by companyname and keep latest date_updated
  const groupedPosts = useMemo(() => {
    const map = new Map<string, Post>();

    for (const post of posts) {
      const existing = map.get(post.companyname);
      if (!existing || new Date(post.date_updated) > new Date(existing.date_updated)) {
        map.set(post.companyname, post);
      }
    }
  ...
  // ✅ Apply date filter on date_created (optional: change to date_updated)
  const filteredPosts = useMemo(() => {
    const start = parseDate(startDate);
    const end = parseDate(endDate);

    return groupedPosts.filter((post) => {
      const postDate = parseDate(post.date_updated);
      return (!start || !postDate || postDate >= start) &&
             (!end || !postDate || postDate <= end);
    });
  }, [groupedPosts, startDate, endDate]);
        ...
        <table className="min-w-full table-auto">
          <thead className="bg-gray-100">
            <tr className="text-xs text-left whitespace-nowrap border-l-4 border-orange-400">
              ...
              <th className="px-6 py-4 font-semibold text-gray-700">Last Purchased</th>
            </tr>
          </thead>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((post) => (
                <tr key={post.id} className="border-b whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 text-xs text-gray-600">{formatDate(post.date_updated)}</td>
                </tr>
              ))
            )}
          </tbody>
