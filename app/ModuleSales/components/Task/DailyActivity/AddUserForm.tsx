    ...
    <>
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-4 text-xs">
        <div className="flex justify-end gap-2">
          ...
          <button type="button" className="text-black border px-4 py-2 rounded text-xs flex items-center gap-1" onClick={onCancel}><CiTurnL1 size={15} /> Back</button>
        </div>
                ...
                {currentRecords.length > 0 ? (
                  currentRecords.map((activity, index) => (
                    <tr key={index} className="border-b whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 text-xs">{formatDate(new Date(activity.callback).getTime())}</td>
                      ...
                      <td className="px-6 py-4 break-words truncate max-w-xs cursor-pointer capitalize" onClick={() => handleShowRemarks(activity.remarks)}>
                        {activity.remarks}
                      </td>
              ...
              <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50 z-[999]">
                <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-lg">
                  ...
                  <div className="modal-body">
                    <p className="text-xs capitalize break-words">{modalRemarks}</p>
                  </div>
                  <button
                    onClick={handleCloseModal}
                    className="mt-4 text-blue-500 px-4 py-2 border rounded-md text-xs"
                  >
                    Close
                  </button>
                </div>
