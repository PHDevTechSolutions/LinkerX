...
import SearchFilters from "../../components/WebsiteInquiry/SearchFilters";
import Inquirytable from "../../components/WebsiteInquiry/InquiryTable";
...
const AccountListPage: React.FC = () => {
    ...
    const fetchAccounts = async () => {
        try {
            const response = await fetch("/api/ModuleCSR/WebsiteInquiry/FetchInquiry"); // Correct API route
    ...
    const filteredAccounts = posts.filter((post) => {
        return (
            (post.CustomerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                post.EmailAddress.toLowerCase().includes(searchTerm.toLowerCase()) ||
                post.CompanyName.includes(searchTerm)) 
        );
    });
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-1">
                                {showForm ? (
                                    ...
                                    <>
                                        ...
                                        <h2 className="text-lg font-bold mb-2">Website Portal Inquiries</h2>
                                        <p className="text-xs mb-4">
                                            This section provides an overview of customer quotations, allowing users to search, filter, and manage records efficiently. Users can refine the results by entering a search term, selecting a city address, or adjusting the number of records displayed per page.
                                        </p>
